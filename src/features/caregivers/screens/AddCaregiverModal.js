...
export default function AddCaregiverModal({ visible, onClose, onSave }) {
    ...
    useEffect(() => {
        const cuidador = cuidadoresAutorizados.find((c) => c.id === selectedCaregiverId);
    ...
    return (
        ...
        <View style={styles.modalContainer}>
            ...
            <View style={styles.headerLeft}>
                <MaterialCommunityIcons name="hand-heart" size={24} color="#4caf50" style={styles.icon} />
        ...
        <View style={styles.infoBox}>
            <MaterialCommunityIcons 
                name="alert-circle" size={20} color="#ab47bc" style={styles.infoIcon} 
            />
            <Text style={styles.infoText}>
                Apenas cuidadores já cadastrados como usuários do aplicativo Mediway – Cuidador podem ser adicionados à rede de apoio.
            </Text>
        </View>
        ...
        >
            <Picker.Item label="Selecione um cuidador..." value="" />
            {
                cuidadoresAutorizados.map((cuidador) => (
                    <Picker.Item key={cuidador.id} label={cuidador.nome} value={cuidador.id} />
                ))
            }
        </Picker>
        </View>
        ...
        <View style={styles.detailsBox}>
            <Text style={styles.detailText}>👤 Nome: {caregiverSelecionado.nome}</Text>
            <Text style={styles.detailText}>🏷️ Tipo: {caregiverSelecionado.tipo}</Text>
        </View>
        )}
